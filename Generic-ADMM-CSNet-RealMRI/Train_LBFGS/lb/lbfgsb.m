...
%             k,history(k,1),history(k,2) );
        if ~isinf(printEvery) && ~mod(k,printEvery)
            ...
            net = weiTOnet(x);
            err = history(k, 1);
            %vloss = Valid_yloss( net);
            save(strcat('./Train_output/net/net-', saveName(k, 5), '.mat'), 'net');
            save(strcat('./Train_output/err/err-', saveName(k, 5), '.mat'), 'err');
            %save(strcat('./output/vloss/vloss-', saveName(k, 5), '.mat'), 'vloss');
        end
        k = outerIter;
    end
...
fprintf('Iter %5d, f(x) = %f, ||grad||_infty = %f', ...
    k, history(k,1),  history(k,2) );
