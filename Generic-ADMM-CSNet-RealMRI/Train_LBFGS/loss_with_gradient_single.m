   ...
   for i = N : -1 : 1
        ...
        switch l.type
            ...
            case 'Non_linear'
                 ...
                 [res(i).dzdx{1}, res(i).dzdw{1}] = nonlinear( LL , res(i).x ,  r,  res(i+1).dzdx);
            case 'relu'
                 if l.leak > 0, leak = {'leak', l.leak} ; else leak = {} ; end
                 if ~isempty(res(i).x)
        ...
        case 'ADD'
             [res(i).dzdx{1}, res(i).dzdx{2}] = Add (res(i).x, res(i-1).x,  res(i+1).dzdx,   res(i+4).dzdx{2}    ) ;
        case 'Remid'
        ...
        case 'Reorg'
             [ res(i).dzdx, res(i).dzdw{1}] = xorg (res(i).x , l.weights{1}, res(i+1).dzdx ,res(i+4).dzdx{2},res(i+5).dzdx{1});
        case 'c_conv'
             w1 = l.weights{1} ;
             w2 = l.weights{2} ; 
             if gp    
